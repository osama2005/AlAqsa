...
function getColorBtn(item, section, i) {
    const c = item._color || '';
    return `<button class="action-btn color-btn" onclick="showRowColorPicker('${section}',${i})" style="background:${c || '#eee'};border:2px solid ${c || '#ccc'};border-radius:50%;width:28px;height:28px;font-size:10px;display:inline-flex;align-items:center;justify-content:center" title="لون الصف">🎨</button>`;
}
...
function showRowColorPicker(section, index) {
    ...
    const swatch = (c, sel) => `<button onclick="setRowColor('${section}',${index},'${c}')" style="width:36px;height:36px;border-radius:6px;border:${c===sel?'3px solid #333':'2px solid #ddd'};background:${c||'#fff'};cursor:pointer;font-size:14px">${c?'':'❌'}</button>`;
    openModal(`لون الصف - ${esc(item.name||item.code||'')}`, `
        ...
        <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:15px">
            <button onclick="setRowTextColor('${section}',${index},'')" style="width:36px;height:36px;border-radius:6px;border:2px solid #ddd;background:#fff;cursor:pointer;font-size:14px" title="إزالة">❌</button>
...
function renderCpBody(sect) {
    ...
    return `
        <div style="margin-bottom:15px;padding:12px;background:#f9f9f9;border-radius:8px">
            <h4 style="margin-bottom:8px;color:#1a7a4c">🟦 الصفوف (الخلفية + النص)</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                ...
                <div style="display:flex;gap:6px;align-items:end;flex-wrap:wrap">
                    ...
                    <button onclick="clearCpRow()" style="padding:6px 10px;font-size:11px;background:#e0e0e0;border:none;border-radius:6px;cursor:pointer">🔄 إزالة</button>
                </div>
            </div>
        </div>
        <div style="margin-bottom:15px;padding:12px;background:#f9f9f9;border-radius:8px">
            <h4 style="margin-bottom:8px;color:#1a7a4c">🟩 الأعمدة (لون النص)</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                ...
                <div style="display:flex;gap:6px;align-items:end;flex-wrap:wrap">
                    ...
                    <button onclick="clearCpCol()" style="padding:6px 10px;font-size:11px;background:#e0e0e0;border:none;border-radius:6px;cursor:pointer">🔄 إزالة</button>
                </div>
            </div>
        </div>
        <div style="padding:12px;background:#f9f9f9;border-radius:8px">
            <h4 style="margin-bottom:8px;color:#1a7a4c">🟪 الخلايا (لون النص)</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
                ...
                <div style="display:flex;gap:6px;align-items:end;flex-wrap:wrap">
                    ...
                    <button onclick="clearCpCell()" style="padding:6px 10px;font-size:11px;background:#e0e0e0;border:none;border-radius:6px;cursor:pointer">🔄 إزالة</button>
                </div>
            </div>
        </div>
    `;
...
// ---- Auto-fill ----
function autoFillItem(codeField, targetFields, source) {
    const code = document.getElementById(codeField).value.trim();
    const item = source.find(i => i.code === code);
    (item ? targetFields : []).forEach(f => {
        const el = document.getElementById(f.id);
        if (el) el.value = item ? f.prop(item) : '';
    });
    if (!item) {
...
function autoFillInventory() {
    const code = document.getElementById('incoming_code').value.trim();
    const item = inventory.find(i => i.code === code);
    ['incoming_ministry_code','incoming_name','incoming_unit','incoming_category','incoming_type']
        .forEach(id => document.getElementById(id).value = item ? item[id.replace('incoming_','')] || item[id.replace('incoming_','ministryCode')] || '' : '');
}

function autoFillOutgoing() {
    const code = document.getElementById('outgoing_code').value.trim();
    const item = inventory.find(i => i.code === code);
    ['outgoing_ministry_code','outgoing_name','outgoing_unit','outgoing_category','outgoing_type']
        .forEach(id => document.getElementById(id).value = item ? item[id.replace('outgoing_','')] || item[id.replace('outgoing_','ministryCode')] || '' : '');
}

function autoFillLoan() {
    const code = document.getElementById('loan_code').value.trim();
    const item = inventory.find(i => i.code === code);
    ['loan_ministry_code','loan_name','loan_unit','loan_category']
        .forEach(id => document.getElementById(id).value = item ? item[id.replace('loan_','')] || item[id.replace('loan_','ministryCode')] || '' : '');
}
    ...
    tbody.innerHTML = inventory.map((item, i) => {
        ...
        return `<tr ${rowStyle(item)}>
            ...
            <td>${canEdit ? `
                ...
                <button class="action-btn edit-btn" onclick="editInventory(${i})">✏️</button>
                <button class="action-btn delete-btn" onclick="deleteInventory(${i})">🗑️</button>` : ''}
            </td>
        </tr>`;
    }).join('');
}
...
function renderIncoming() {
    ...
    tbody.innerHTML = incoming.map((rec, i) => {
        ...
        <td>${canEdit ? `
            ...
            <button class="action-btn edit-btn" onclick="editIncoming(${i})">✏️</button>
            <button class="action-btn delete-btn" onclick="deleteIncoming(${i})">🗑️</button>` : ''}
        </td>
    </tr>`}).join('');
}
...
function renderOutgoing() {
    ...
    tbody.innerHTML = outgoing.map((rec, i) => {
        ...
        <td>${canEdit ? `
            ...
            <button class="action-btn edit-btn" onclick="editOutgoing(${i})">✏️</button>
            <button class="action-btn delete-btn" onclick="deleteOutgoing(${i})">🗑️</button>` : ''}
        </td>
    </tr>`}).join('');
}
    ...
    tbody.innerHTML = loanItems.map((item, i) => {
        ...
        return `<tr ${rowStyle(item)}>
            ...
            <td>${canEdit ? `
                ...
                <button class="action-btn edit-btn" onclick="editLoan(${i})">✏️</button>
                <button class="action-btn delete-btn" onclick="deleteLoan(${i})">🗑️</button>` : ''}
            </td>
        </tr>`;
    }).join('');
}
...
function renderLoanIncoming() {
    ...
    tbody.innerHTML = loanIncoming.map((rec, i) => {
        ...
        <td>${canEdit ? `
            ...
            <button class="action-btn edit-btn" onclick="editLoanIncoming(${i})">✏️</button>
            <button class="action-btn delete-btn" onclick="deleteLoanIncoming(${i})">🗑️</button>` : ''}
        </td>
    </tr>`}).join('');
}
...
function renderKahana() {
    ...
    tbody.innerHTML = kahana.map((item, i) => {
        ...
        <td>${canEdit ? `
            ...
            <button class="action-btn edit-btn" onclick="editKahana(${i})">✏️</button>
            <button class="action-btn delete-btn" onclick="deleteKahana(${i})">🗑️</button>` : ''}
        </td>
    </tr>`}).join('');
}
